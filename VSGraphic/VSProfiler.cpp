...
void VSProfiler::Draw(VSRenderer * pRenderer)
{
#ifdef PROFILER
	ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER(VSDrawProfileCommand, VSRenderer *, pRenderer, pRenderer,
	{
		for (unsigned int i = 0; i < m_pRoot.GetNum(); i++)
		{
			m_pRoot[i]->Draw(pRenderer, 0);
		}
		ClearAll();
...
void VSTimeProfilerNode::OnDraw(VSRenderer * pRenderer, unsigned int uiLayer)
{
	VSString Out;
	VSProfiler::GetProfilerNameString(m_Name, uiLayer, Out);
	Out += _T(":%fms");
	VSRenderer::ms_pRenderer->DrawTextForProfile(NULL, 0, VSProfiler::ms_uiCurCount * 20, VSColorRGBA(1.0f, 1.0f, 0.0f), Out.GetBuffer(), m_fProfilerTime);
}
...
void VSCountProfilerNode::OnDraw(VSRenderer * pRenderer, unsigned int uiLayer)
{
	VSString Out;
	VSProfiler::GetProfilerNameString(m_Name, uiLayer, Out);
	Out += _T(":%d");
	VSRenderer::ms_pRenderer->DrawTextForProfile(NULL, 0, VSProfiler::ms_uiCurCount * 20, VSColorRGBA(1.0f, 1.0f, 0.0f), Out.GetBuffer(), m_uiCounter);
}
...
void VSOnlyTimeProfilerNode::OnDraw(VSRenderer * pRenderer, unsigned int uiLayer)
{
	VSString Out;
	VSProfiler::GetProfilerNameString(m_Name, uiLayer, Out);
	Out += _T(":%f");
