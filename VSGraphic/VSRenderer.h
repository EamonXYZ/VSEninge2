...
namespace VSEngine2
{
	...
	public:
		...
		void DrawText(VSFont* pFont,int iX, int iY, const VSColorRGBA& rColor,const TCHAR* acText, ...);		
		virtual void DrawTextForProfile(VSFont* pFont, int iX, int iY, const VSColorRGBA& rColor, const TCHAR* acText, ...) = 0;
		virtual bool SetViewPort(VSViewPort *pViewPort,unsigned int uiRtWidth = 0,unsigned int uiRtHeight = 0) = 0;

		

		FORCEINLINE const VSFont *GetFont()const;
		...
		FORCEINLINE unsigned int GetMaxActiveLight()const;
		unsigned int GetMaxUseClipPlane()const;
		...
		FORCEINLINE unsigned int GetVertexShaderVersion()const;
		FORCEINLINE unsigned int GetPixelShaderVersion()const;
		...
		bool LoadFont(VSFont *pFont);
		bool ReleaseFont(VSFont *pFont);
		...
		virtual bool DrawMesh(VSGeometry * pGeometry,VSRenderState * pRenderState,VSVShader * pVShader, VSPShader * pPShader) = 0;
		...
		virtual unsigned int SetBlendState(VSBlendState * pBlendState) = 0;
		virtual unsigned int SetDepthStencilState(VSDepthStencilState * pDepthStencilState) = 0;
		virtual unsigned int SetRasterizerState(VSRasterizerState * pRasterizerState) = 0;
		...
		virtual unsigned int SetSamplerState(VSSamplerState * pSamplerState,unsigned int i) = 0;

		virtual bool SetRenderTarget(VSRenderTarget *pRenderTarget,unsigned int i) = 0;
		virtual bool EndRenderTarget(VSRenderTarget *pRenderTarget,unsigned int i) = 0;
		...
		void EnableTextureRTTextureUsed(VSTexture * pTexture);
		void DisableTextureRTTextureUsed(VSTexture * pTexture);
...
// 		virtual bool SetVProgramConstant (VSString &NameInShaderProgram,void * pDate,unsigned int uiSize) = 0;
// 		virtual bool SetPProgramConstant (VSString &NameInShaderProgram,void * pDate,unsigned int uiSize) = 0;

		virtual bool SetVProgramConstant (unsigned int uiStartRegister,void * pDate,
											unsigned int RegisterNum,unsigned int uiType) = 0;
		...
		virtual bool OnReleasePShaderProgram (VSResourceIdentifier* pPShaderProgramID) = 0;
		...
		virtual bool OnLoadFont(VSFont * pFont, VSResourceIdentifier *&pID) = 0;
		virtual bool OnReleaseFont(VSResourceIdentifier * pFontID) = 0;
		...
		virtual void OnDrawText(int iX, int iY, const VSColorRGBA& rColor,VSString & String) = 0;
	...
	//硬件信息和处理能力
		...
		UINT		m_uiMaxActiveLight;
		UINT		m_uiMaxUseClipPlane;
		UINT		m_uiMaxTexture;
		UINT		m_uiMaxVTexture;
		UINT		m_uiMaxAnisotropy;
		UINT		m_uiVertexShaderVersion;
		UINT		m_uiPixelShaderVersion;
		UINT		m_uiMaxRTNum;
	...
	public:
		...
		virtual VSString Sampler(unsigned int uiType)const = 0;
		...
		virtual VSString SetRegister(unsigned int uiRegisterType,unsigned int uiRegisterIndex)const = 0;
