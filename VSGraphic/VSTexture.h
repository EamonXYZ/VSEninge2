...
namespace VSEngine2
{
	...
	public:
		...
		FORCEINLINE unsigned char * GetBuffer(unsigned int uiLevel,unsigned int i)const;
		virtual unsigned int GetByteSize(unsigned int uiLevel)const;

		FORCEINLINE unsigned int GetWidth(unsigned int uiLevel)const;
		FORCEINLINE unsigned int GetHeight(unsigned int uiLevel)const;
		FORCEINLINE unsigned int GetLength(unsigned int uiLevel)const;
		...
		{
			if(m_uiFormatType == VSRenderer::SFT_X8R8G8B8 || m_uiFormatType == VSRenderer::SFT_A8R8G8B8 ||
