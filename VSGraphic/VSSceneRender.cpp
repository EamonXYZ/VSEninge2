...
void VSPostEffectSceneRender::GetRT(unsigned int uiWidth,unsigned int uiHeight)
{
	...
	VSRenderTarget * pRenderTarget = VSResourceManager::Get2DRenderTarget(uiWidth,uiHeight,VSRenderer::SFT_X8R8G8B8,0);
...
bool VSProjectShadowSceneRender::OnDraw(VSCuller & Culler,unsigned int uiRenderGroup,double dAppTime)
{
		...
		VSPShader * pPShader = NULL;
		{
			...
			if(!VSRenderer::ms_pRenderer->DrawMesh(VSGeometry::GetDefaultCub(),&pCustomMaterial->GetRenderState(),pVShader,pPShader))
			{
				return false;
			}
		}

		{
			...
			static VSUsedName ShadowSamplerString = _T("ShadowSampler");
