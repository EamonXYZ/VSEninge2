...
unsigned int VSDX9Renderer:: ms_uiVertexShaderProgramVersion = ShaderVersion(3,0);
unsigned int VSDX9Renderer::ms_uiPixelShaderProgramVersion = ShaderVersion(3,0);
...
DWORD VSDX9Renderer::ms_dwAlphaTest[] =
{
	D3DCMP_NEVER,
	D3DCMP_LESS,
	D3DCMP_EQUAL,
...
DWORD VSDX9Renderer::ms_dwFogComputeMethod[] =
{
	D3DFOG_LINEAR,
	D3DFOG_EXP,
	D3DFOG_EXP2
...
DWORD VSDX9Renderer::ms_dwLightType[] =
{
	0,
	D3DLIGHT_POINT,
	D3DLIGHT_SPOT ,
...
unsigned int VSDX9Renderer::SetSamplerState(VSSamplerState * pSamplerState,unsigned int i)
{
	unsigned int FRI = VSRenderer::SetSamplerState(pSamplerState,i);
...
unsigned int VSDX9Renderer::SetBlendState(VSBlendState * pBlendState)
{
	unsigned int FRI = VSRenderer::SetBlendState(pBlendState);
	if(FRI == FRI_SAMERESOURCE)
		return FRI_SUCCESS;
	VSMAC_ASSERT(FRI == FRI_SUCCESS);
	if (FRI == FRI_FAIL)
...
unsigned int VSDX9Renderer::SetDepthStencilState(VSDepthStencilState * pDepthStencilState)
{
	unsigned int FRI = VSRenderer::SetDepthStencilState(pDepthStencilState);
	if(FRI == FRI_SAMERESOURCE)
		return FRI_SUCCESS;
	VSMAC_ASSERT(FRI == FRI_SUCCESS);
	if (FRI == FRI_FAIL)
...
unsigned int VSDX9Renderer::SetRasterizerState(VSRasterizerState * pRasterizerState)
{
	unsigned int FRI = VSRenderer::SetRasterizerState(pRasterizerState);
	if(FRI == FRI_SAMERESOURCE)
		return FRI_SUCCESS;
	VSMAC_ASSERT(FRI == FRI_SUCCESS);
	if (FRI == FRI_FAIL)
...
// 			VSMAC_ASSERT(!FAILED(hResult));
	...
	if (b2)
	{
		CreateRenderTarget((D3DFORMAT)ms_dwTextureFormatType[pRenderTarget->GetFormatType()],(D3DMULTISAMPLE_TYPE)ms_dwMultiSampleTypes[pRenderTarget->GetMulSample()],
...
bool VSDX9Renderer::OnLoadDepthStencil (VSDepthStencil * pDepthStencil,VSResourceIdentifier *&pID)
{
	...
	if(!pDepthStencilID)
		return 0;
	pID = pDepthStencilID;
	CreateDepthStencilSurface((D3DFORMAT)ms_dwTextureFormatType[pDepthStencil->GetFormatType()],(D3DMULTISAMPLE_TYPE)ms_dwMultiSampleTypes[pDepthStencil->GetMulSample()],
...
bool VSDX9Renderer::OnLoadFont(VSFont * pFont, VSResourceIdentifier *&pID)
{
	VSFontID *pFontID = NULL;
	pFontID = VS_NEW VSFontID;
	if(pFontID == NULL)
		return 0;
	pID = pFontID;

	DWORD dwWeight = (pFont->GetBold() ? FW_BOLD : FW_REGULAR);
	CreateFont(pFont->GetSize(),dwWeight,pFont->GetItalic(),pFont->GetFace().GetBuffer(),&pFontID->m_pFont);
// 	HRESULT hResult = NULL;
// 	
// 	hResult = D3DXCreateFont(
// 		m_pDevice,                     // pDevice
// 		pFont->GetSize(),               // Height
// 		0,                              // Width
// 		dwWeight,                       // Weight
// 		0,                              // MipLevels
// 		pFont->GetItalic(),				// Italic
// 		DEFAULT_CHARSET,                // CharSet
// 		OUT_DEFAULT_PRECIS,             // OutputPrecision
// 		ANTIALIASED_QUALITY,            // Quality
// 		VARIABLE_PITCH,                 // PitchAndFamily
// 		pFont->GetFace().GetBuffer(),       // pFaceName
// 		&pFontID->m_pFont);             // ppFont
// 
// 	VSMAC_ASSERT(!FAILED(hResult));

	return 1;
}
bool VSDX9Renderer::OnReleaseFont(VSResourceIdentifier * pFontID)
{
//	VSFontID * pFontID = (VSFontID *) pFont;

	VSMAC_DELETE(pFontID);
	return 1;
}
...
void VSDX9Renderer::OnDrawText(int iX, int iY, const VSColorRGBA& rColor,VSString & String)
{
	VSFontID* pFontID = (VSFontID *)m_pFont->GetIdentifier();
	VSMAC_ASSERT(pFontID);
	DrawText(pFontID->m_pFont,iX,iY,rColor,String);
}
unsigned int VSDX9Renderer::GetTextureCompressLen(unsigned int uiLen)
{
	return Max((unsigned int)1,(unsigned int)(uiLen / 4));
}
// bool VSDX9Renderer::SetVProgramConstant (VSString &NameInShaderProgram,void * pDate,unsigned int uiSize)
// {
// 	VSVProgramID * pVProgramID =(VSVProgramID *)m_pVShader->GetIdentifier();
// 	if(!pVProgramID)
// 		return 0;
// 	hResult = pVProgramID->m_pConstantTable->SetValue(m_pDevice,NameInShaderProgram.GetBuffer(),pDate,uiSize);
// 	VSMAC_ASSERT(!FAILED(hResult));
// 	return 1;
// 
// }
// bool VSDX9Renderer::SetPProgramConstant (VSString &NameInShaderProgram,void * pDate,unsigned int uiSize)
// {
// 	VSPProgramID *pPProgramID =(VSPProgramID *)m_pPshader->GetIdentifier();
// 	if(!pPProgramID)
// 		return 0;
// 
// 	hResult = pPProgramID->m_pConstantTable->SetValue(m_pDevice,NameInShaderProgram.GetBuffer(),pDate,uiSize);
// 	VSMAC_ASSERT(!FAILED(hResult));
// 	return 1;
// }
...
bool VSDX9Renderer::DrawMesh(VSGeometry * pGeometry,VSRenderState * pRenderState,VSVShader * pVShader, VSPShader * pPShader)
{
	
	ADD_TIME_PROFILE(DrawMesh);
	if (!VSRenderer::DrawMesh(pGeometry, pRenderState, pVShader, pPShader))
		return 0;
...
bool VSDX9Renderer::SetViewPort(VSViewPort *pViewPort,unsigned int uiRtWidth,unsigned int uiRtHeight)
{
	D3DVIEWPORT9 View;
	if (!uiRtWidth)
	{
		uiRtWidth = m_uiCurRTWidth;
	}
	if (!uiRtHeight)
	{
		uiRtHeight = m_uiCurRTHeight;
	}
	if (pViewPort)
	{
		VSViewPort ViewPort;
		ViewPort = *pViewPort;
		View.X = Rounding(ViewPort.XMin * uiRtWidth);
		View.Y = Rounding(ViewPort.YMin * uiRtHeight);
		View.Width = Rounding(ViewPort.XMax * uiRtWidth) - View.X;
		View.Height = Rounding(ViewPort.YMax * uiRtHeight) - View.Y;
	...
	else
	{
		View.X = 0;
		View.Y = 0;
		View.Width = uiRtWidth;
		View.Height = uiRtHeight;
		View.MinZ = 0.0f;
		View.MaxZ = 1.0f;
							  ...
							  ChildWindowInfo * pChildWindow,int uiNumChildWindow)
{
	...
	m_uiMaxActiveLight = d3dCaps.MaxActiveLights;
	m_uiMaxTexture = 16;
	...
	m_uiVertexShaderVersion = d3dCaps.VertexShaderVersion;	
...
bool VSDX9Renderer::InitialDefaultState()
{
	...
	for(unsigned int nAdapter = 0 ; nAdapter < ms_uiAdapterNum; nAdapter++)
	{
		...
		unsigned int DisplayModeNumX8R8G8B8 = 0;
		unsigned int DisplayModeNumR5G6B5	= 0;
		DisplayModeNumX8R8G8B8 = ms_pMain->GetAdapterModeCount(nAdapter, (D3DFORMAT)ms_dwTextureFormatType[VSRenderer::SFT_X8R8G8B8]);
		DisplayModeNumR5G6B5 = ms_pMain->GetAdapterModeCount(nAdapter, (D3DFORMAT)ms_dwTextureFormatType[VSRenderer::SFT_R5G6B5]);
		unsigned int DisplayModeNum = DisplayModeNumX8R8G8B8 + DisplayModeNumR5G6B5;
		ms_AdapterInfo[nAdapter].uiDisplayModeNum = DisplayModeNum;		
		ms_AdapterInfo[nAdapter].DisplayMode.Clear();
		ms_AdapterInfo[nAdapter].DisplayMode.SetBufferNum(DisplayModeNum);
		for (UINT nMode = 0; nMode < DisplayModeNumX8R8G8B8; nMode++) 
		{
			TCHAR temp[20];
			hResult = ms_pMain->EnumAdapterModes(nAdapter,(D3DFORMAT)ms_dwTextureFormatType[VSRenderer::SFT_X8R8G8B8],nMode, &d3ddspmd);
			VSMAC_ASSERT(!FAILED(hResult));
			ms_AdapterInfo[nAdapter].DisplayMode[nMode].uiBitFormat = 32;
		...
		for (UINT nMode = 0; nMode < DisplayModeNumR5G6B5; nMode++) 
		{
			TCHAR temp[20];
			hResult = ms_pMain->EnumAdapterModes(nAdapter,(D3DFORMAT)ms_dwTextureFormatType[VSRenderer::SFT_R5G6B5],nMode, &d3ddspmd);
			VSMAC_ASSERT(!FAILED(hResult));
			ms_AdapterInfo[nAdapter].DisplayMode[nMode + DisplayModeNumX8R8G8B8].uiBitFormat = 16;
			ms_AdapterInfo[nAdapter].DisplayMode[nMode + DisplayModeNumX8R8G8B8].uiHeight = d3ddspmd.Height;
			ms_AdapterInfo[nAdapter].DisplayMode[nMode + DisplayModeNumX8R8G8B8].uiWidth = d3ddspmd.Width;
			ms_AdapterInfo[nAdapter].DisplayMode[nMode + DisplayModeNumX8R8G8B8].uiRefreshRate = d3ddspmd.RefreshRate;
			sprintf_s(temp,"%d * %d * 16",d3ddspmd.Width,d3ddspmd.Height);
			ms_AdapterInfo[nAdapter].DisplayMode[nMode + DisplayModeNumX8R8G8B8].StringExpress = temp;
		}
		for(unsigned int DevType = 0; DevType < 2 ; DevType++)
		{
...
void VSDX9Renderer::DeviceLost()
{
	...
	if(!pFontID)
		return ;
	HRESULT hResult = NULL;
	hResult = pFontID->m_pFont->OnLostDevice();
	VSMAC_ASSERT(!FAILED(hResult));	
...
void VSDX9Renderer::ResetDevice()
{
	...
	if(!pFontID)
		return ;
	hResult = pFontID->m_pFont->OnResetDevice();
	VSMAC_ASSERT(!FAILED(hResult));	
