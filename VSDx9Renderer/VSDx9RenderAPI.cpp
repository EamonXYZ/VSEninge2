...
DECLEAR_COUNT_PROFILENODE(DrawPassNum, )
...
bool VSDX9Renderer::DrawDynamicBufferMesh(VSDynamicBufferGeometry *pDBGeometry)
{
		...
		VSGeometry *,pDBGeometry,pDBGeometry,LPDIRECT3DDEVICE9,m_pDevice,m_pDevice,
	{
		...
		while(true)
		{
			...
			ADD_COUNT_PROFILE(DrawPassNum, 1)
...
bool VSDX9Renderer::DrawMesh(VSGeometry * pGeometry)
{
		...
		VSGeometry *,pGeometry,pGeometry,LPDIRECT3DDEVICE9,m_pDevice,m_pDevice,
	{
		...
		ADD_COUNT_PROFILE(DrawPrimitiveNum, pGeometry->GetActiveNum())
		VSMAC_ASSERT(!FAILED(hResult));
...
bool VSDX9Renderer::DrawMesh1(VSGeometry * pGeometry)
{
		...
		VSDrawMesh1Para ,DrawMesh1Para,DrawMesh1Para,LPDIRECT3DDEVICE9,m_pDevice,m_pDevice,
	{
		...
		ADD_COUNT_PROFILE(DrawPrimitiveNum, DrawMesh1Para.uiActiveNum)
		VSMAC_ASSERT(!FAILED(hResult));
...
void VSDX9Renderer::DrawTextForProfile(VSFont* pFont, int iX, int iY, const VSColorRGBA& rColor, const TCHAR* acText, ...)
{
	if (!acText)
		return;
	if (!pFont)
	...
	D3DCOLOR kColor = rColor.GetDWARGB();
	HRESULT hResult = NULL;
	hResult = pFontID->m_pFont->DrawText(
		0,                                  // pSprite
		String.GetBuffer(),                 // pString
		-1,                                 // Count
		&kText,                             // pRect
		DT_LEFT | DT_BOTTOM | DT_NOCLIP,  // Format
		kColor);
	VSMAC_ASSERT(!FAILED(hResult));
}
...
bool VSDX9Renderer::DrawText(LPD3DXFONT pFont,int iX, int iY, const VSColorRGBA& rColor,VSString & String)
{
	...
	struct VSDx9DrawTextPara
	{
		D3DCOLOR kColor;
		RECT kText;
		VSString String;
	};
	VSDx9DrawTextPara DrawTextPara;
	DrawTextPara.kColor = kColor;
	DrawTextPara.kText = kText;
	DrawTextPara.String = String;
	HRESULT hResult = NULL;

	ENQUEUE_UNIQUE_RENDER_COMMAND_TWOPARAMETER(VSDx9DrawTextCommand,
		VSDx9DrawTextPara,DrawTextPara,DrawTextPara,LPD3DXFONT,pFont,pFont,
	{
		HRESULT hResult = NULL;
		hResult = pFont->DrawText(
			0,                                  // pSprite
			DrawTextPara.String.GetBuffer(),                 // pString
			-1,                                 // Count
			&DrawTextPara.kText,                             // pRect
			DT_LEFT | DT_BOTTOM | DT_NOCLIP,  // Format
			DrawTextPara.kColor);    
		VSMAC_ASSERT(!FAILED(hResult));
...
void VSDX9Renderer::CreateFont(INT Height,UINT Weight,BOOL Italic,const TCHAR *pFaceName,LPD3DXFONT*  ppFont)
{
	struct VSCreateFontPara
	{
		INT Height;
		UINT Weight;
		BOOL Italic;
	...
	VSCreateFontPara CreateFontPara;
	CreateFontPara.FaceName = pFaceName;
	CreateFontPara.Height = Height;
	CreateFontPara.Italic = Italic;
	CreateFontPara.ppFont = ppFont;
	CreateFontPara.Weight = Weight;
	ENQUEUE_UNIQUE_RENDER_COMMAND_TWOPARAMETER(VSCreateFontCommand,
		VSCreateFontPara,CreateFontPara,CreateFontPara,LPDIRECT3DDEVICE9,m_pDevice,m_pDevice,
	{
		HRESULT hResult = NULL;
		hResult = D3DXCreateFont(
			...
			CreateFontPara.FaceName.GetBuffer(),       // pFaceName
			CreateFontPara.ppFont);             // ppFont
